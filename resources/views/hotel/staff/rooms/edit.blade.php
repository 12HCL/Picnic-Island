...
<nav aria-label="breadcrumb" class="mb-3">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Edit Room {{ $room->room_number }}</li>
    </ol>
</nav>
    ...
    <div class="col-lg-6">
        <div class="card shadow-sm">
            <div class="card-header bg-primary text-white">
                <h1 class="h5 mb-0 text-white">Edit Room {{ $room->room_number }}</h1>
            </div>
            <div class="card-body">
                <form action="{{ route('hotel.staff.rooms.update', $room) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <select name="hotel_id" id="hotel_id" class="form-select @error('hotel_id') is-invalid @enderror" required>
                            @foreach ($hotels as $h)
                                <option value="{{ $h->id }}" {{ old('hotel_id', $room->hotel_id) == $h->id ? 'selected' : '' }}>
                                    {{ $h->name }}
                                </option>
                            @endforeach
                        </select>
